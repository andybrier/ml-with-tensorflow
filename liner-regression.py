...
with tf.Session() as sess:
    ...
    for iteration in range(training_iteration):
        ...
        if iteration % display_step == 0:
           print("Iteration: {0}, cost:{1}, W={2}, b={3}".format(iteration + 1, sess.run(cost_function,  feed_dict={   X: size_data_n,  Y: price_data_n}),  sess.run(W), sess.run(b)))

    tuning_cost = sess.run(cost_function, feed_dict={X: normalize(size_data_n), Y: normalize(price_data_n)})

    print("Tuning completed, cost={0}, w={1}, b={2}".format(tuning_cost, sess.run(W), sess.run(b)))
    # Validate a tuning model

    testing_cost = sess.run(cost_function, feed_dict={X: size_data_test_n, Y: price_data_test_n})

    print("Testing data cost: {0}" .format(testing_cost))
